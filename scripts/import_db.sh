...
get_import_command() {
  echo "\\copy $1$version from program 'gzip -dc $input_folder/$1$version.csv.gz' csv header;"
}

conservation=`get_import_command "conservation"`
gene=`get_import_command "gene"`
job_ena="\\copy job_ena$version(run_accession,status,created_at,queued_at,downloaded_at,analysed_at,cleaned_at,loaded_at,cooccurrence_at,failed_at,error_message,fastq_path,vcf_path,qc_path,horizontal_coverage_path,vertical_coverage_path,num_reads,covered_bases,coverage,mean_depth,mean_base_quality,mean_mapping_quality) from program 'gzip -dc $input_folder/job_ena$version.csv.gz' csv header;"
...
sample_gisaid="\\copy sample_gisaid$version(run_accession,date,host_tax_id,host,country_raw,region,country,country_alpha_2,country_alpha_3,continent,continent_alpha_2,site,site2,sequence_length,count_n_bases,count_ambiguous_bases,count_snvs,count_insertions,count_deletions) from program 'gzip -dc $input_folder/sample_gisaid$version.csv.gz' csv header;"
