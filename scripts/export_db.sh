...
get_export_command() {
  echo "\\copy $1$version to program 'gzip > $output/$1$version.csv.gz' csv header;"
}
...
sample_gisaid="\\copy sample_gisaid$version(run_accession,date,host_tax_id,host,country_raw,region,country,country_alpha_2,country_alpha_3,continent,continent_alpha_2,site,site2,sequence_length,count_n_bases,count_ambiguous_bases,count_snvs,count_insertions,count_deletions) to program 'gzip > $output/sample_gisaid$version.csv.gz' csv header;"
