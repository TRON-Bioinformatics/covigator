...
class Pipeline:
    ...
    def run(self, run_accession: str, fastq1: str, fastq2: str = None):
        ...
        snpeff_vcf_file_gz = os.path.join(
            sample_data_folder, "{name}.lofreq.normalized.annotated.vcf.gz".format(name=run_accession))
        ...
        command = "nextflow run {workflow} -r {workflow_version} " \
            ...
            fastq1=fastq1, fastq2="--fastq2 " + fastq2 if fastq2 else "", output_folder=sample_data_folder,
            name=run_accession, work_folder=self.config.temp_folder,
            workflow_version=self.config.workflow_version, workflow=self.config.workflow)
